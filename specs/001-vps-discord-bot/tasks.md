...
- [ ] T004 [P] Create `eslint.config.js` (flat config) with the `no-restricted-paths` zones from research R10 — copy the concrete syntax block verbatim from `research.md` R10 (zones for `discord.js`/`pino`/`zod`); invoke `eslint --max-warnings 0` via `npm run lint` and as part of CI
...
- [ ] T026a [US3] Create thin `src/health-cli.ts` wrapper exporting a `main()` that connects to the in-process `BotState` via the running health endpoint (or pulls it from a `--socket`/`--port` flag) and prints `mapHealthStatus(botState)` to stdout; compiles to `dist/health-cli.js` invoked by the `npm run health` script declared in T001. (Optional CLI — the operator can also `curl http://127.0.0.1:8081/healthz`; this wrapper exists for `npm run health` convenience and ties back to the T001 npm script so it does not fail at runtime.)
