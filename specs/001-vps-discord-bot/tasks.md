...
- [ ] T020 [US2] Implement entrypoint in `src/index.ts` — single `runApp().catch(() => { /* fatal already logged upstream; ensure non-zero */ process.exit(1); });` (per `contracts/lifecycle.md`: `node dist/index.js` invokes `runApp`)
...
- [ ] T029 Run `npm run lint` and fix any violations; confirm final zero-warning state — the `no-restricted-paths` zones (T004) are CI-enforced via T031
...
- [ ] T031 Add `.github/workflows/ci.yml` (Node 24, on push + pull_request) running `npm ci`, `npm run lint`, `npm run build`, `npm test` — the CI enforcement point referenced by T004/T029, plan.md "Growth & boundary enforcement", and contracts/discord.md
