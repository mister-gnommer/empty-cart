...
| `tests/integration/health.*` | full `phase × discord` matrix over real HTTP; `shutting-down` returns 503 (no stale healthy) | FR-007, Story 3 #1–#3 |
...
- Entry point & signal handling: `src/lifecycle/` + `src/index.ts` — `contracts/lifecycle.md`.
