...
1. `loadConfig` validates `env` exactly once against the zod schema in `research.md` R5 / `data-model.md` Entity 1. It takes **no logger** — it is a pure validator that either returns `Config` or throws `ConfigError`. `healthHost` is validated as an IPv4 literal specifically (loopback-only on IPv4; IPv6 loopback `::1` is out of scope for v1 — see `data-model.md` Entity 1 §Validation note).
...
- **Cross-module coverage reference (no re-specified test here)**: redaction of `discordToken` in an accidental `log.info({ config })` is asserted by the logger suite — see `contracts/logger.md` Test obligations (depth-2 `*.discordToken` path). This contract does NOT duplicate that obligation; the logger contract owns it.