...
2. **Event → state mapping**:
   ...
   - `Events.ShardDisconnect` → `botState.discord = 'reconnecting'` (or `'disconnected'` if fatal); `log warn: msg="discord shard disconnected"; fields: closeCode, correlationId`. The `correlationId` is a NEW `crypto.randomUUID()` generated at handler entry — it is NOT the `UserCommand.correlationId` (there is no user command in this path); it exists solely to satisfy FR-012 "MUST log each reconnect attempt with a correlation identifier" so an operator can trace a single disconnect→reconnect→resume sequence across multiple log lines.
   - `Events.ShardResumed`/`Events.ShardReady` after disconnect → `botState.discord = 'connected'`; `log info: msg="discord reconnected"; fields: correlationId` carrying the SAME correlationId generated by the preceding `ShardDisconnect` event so the disconnect→resume pair is traceable (FR-012 / Edge Case gateway-down).
...
- A contract test asserts `(5)`: echo handler throw → user-facing error reply sent with the canonical string `"An internal error occurred while processing your command."` (byte-equal — no exception text leaked in the reply) + `error` log line with `msg="command handler threw"`. (FR-011.)
